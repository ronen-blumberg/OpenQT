...
The author is Ronen Blumberg. Versioning is tracked in the `openqt.c` header comment, the `#define VERSION` constant (used by the About box, line ~74), and `make_hlp.py`'s `HEADER` string — currently **Version 3.3.0**. `config.h` and `readme.txt` lag far behind (1.0); the new `README.md` (~36 KB) is the user-facing doc and is current. Treat `openqt.c` and `README.md` as the source of truth when older files disagree. When bumping the version, update all three spots — they tend to drift.
